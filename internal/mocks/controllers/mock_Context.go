...
import (
	context "context"
	config "subscription-api/config"
...
func (_m *Context) Logger() config.Logger {
	ret := _m.Called()
	...
	var r0 config.Logger
	if rf, ok := ret.Get(0).(func() config.Logger); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(config.Logger)
		}
	}
...
func (_c *Context_Logger_Call) Return(_a0 config.Logger) *Context_Logger_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *Context_Logger_Call) RunAndReturn(run func() config.Logger) *Context_Logger_Call {
	_c.Call.Return(run)
	return _c
}
